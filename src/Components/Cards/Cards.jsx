...
const Cards = ({ confirmed, date, recovered, deaths }) => {
	...
	if (!confirmed) {
		return <HashLoader css={override} size={40} color={'#123abc'} loading={loading} />;
	}

	return (
		<div className={styles.container}>
			<Grid container spacing={4} justify="center">
				<Grid item xs={10} md={4} component={Card} className={cx(styles.card, styles.infected)}>
					<CardContent>
				...
				<Grid item xs={10} md={4} component={Card} className={cx(styles.card, styles.active)}>
					<CardContent>
						<Typography className={styles.aheading}>Active Cases</Typography>
						<Typography color="textPrimary" variant="h4">
							<CountUp start={0} end={confirmed - (recovered + deaths)} duration={2} separator={','} />
						</Typography>
						<Typography color="textSecondary">
							Last updated at {(date = new Date().toLocaleTimeString())}
						</Typography>
				...
				<Grid item xs={10} md={4} component={Card} className={cx(styles.card, styles.recovered)}>
					<CardContent>
				...
				<Grid item xs={10} md={4} component={Card} className={cx(styles.card, styles.deaths)}>
					<CardContent>
